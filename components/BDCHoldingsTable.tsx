...
const ASSET_CLASSES = ["All", "1st Lien", "2nd Lien", "Unsecured/Sub", "Equity", "Structured/JV", "Other"];
...
export default function BDCHoldingsTable({ ticker }: { ticker: string }) {
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((h) => {
      if (ac !== "All" && h.asset_class !== ac) return false;
  ...
  const columns: Column<Holding>[] = [
    {
      ...
      render: (h) => (
        <div>
          ...
          {h.legal_name ? <div className="text-xs" style={{ color: "#52526a" }}>{h.legal_name}</div> : null}
        </div>
      ),
    },
    { key: "sector", label: "Sector", sortable: true, render: (h) => <span style={{ color: "#9ca3af" }}>{h.sector ?? "—"}</span> },
    { key: "asset_class", label: "Type", sortable: true, render: (h) => <span style={{ color: "#9ca3af" }}>{h.asset_class}</span> },
    ...
    { key: "mark", label: "Mark", sortable: true, align: "right", render: (h) => <span className="tabular-nums">{markCents(h.mark)}</span> },
    { key: "coupon", label: "Coupon", sortable: false, render: (h) => <span className="text-xs" style={{ color: "#8b8ba8" }}>{h.coupon ?? "—"}</span> },
    { key: "maturity", label: "Maturity", sortable: true, align: "right", render: (h) => <span className="text-xs tabular-nums" style={{ color: "#8b8ba8" }}>{h.maturity ?? "—"}</span> },
  ];
  ...
  return (
    <div className="mb-6">
      <div className="flex flex-wrap items-end justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-xs" style={{ color: "#8b8ba8" }}>
            {ticker}&apos;s largest positions from parsed SOI · top {all.length} by fair value · as of {holdingsAsOf}
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        />
        <select value={ac} onChange={(e) => setAc(e.target.value)} className="px-3 py-1.5 text-sm" style={inputStyle}>
          {ASSET_CLASSES.map((a) => <option key={a} value={a}>{a === "All" ? "All types" : a}</option>)}
        </select>
      ...
      <SortableTable<Holding>
        data={rows}
        columns={columns}
        rowKey={(h) => `${h.name}|${h.type}|${h.maturity}|${h.fv_m}`}
